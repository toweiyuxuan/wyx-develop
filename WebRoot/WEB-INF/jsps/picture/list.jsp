<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="../taglibs.jsp" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
		<title>admin</title>
	</head>
	<body>
		<div class="public_box">
			<h1 class="title">
				<strong>图片列表</strong>
				<div class="articleTop">
					<div class="win-input">
						<input type="search" class="text-serach" placeholder="请输入关键词检索文档">
						<button class="button-normal" id="alertbox">搜索</button>
					</div>
					<span>
						<a class="button-normal button-search" href="#">图片列表</a>
						<a class="button-normal button-add" href="picture-create.html">创建图片</a>
					</span>
				</div>
			</h1>
			<table class="table-default mb10">
				<tr>
					<th width="50">选择</th>
					<th width="60">ID</th>
					<th width="255">图片名称</th>
					<th width="160">链接</th>
					<th width="40">封面</th>
					<th width="80">所属栏目</th>
					<th width="80">打开方式</th>
					<th width="80">状态</th>
					<th width="150">更新时间</th>
					<th>操作</th>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>

					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>

					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>

					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>
					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>
					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>
					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
				<tr>
					<td>
						<label class="label-checkbox">
							<input type="checkbox" name="checkbox">
							<span class="checkbox-name"> </span>
						</label>
					</td>
					<td>1</td>
					<td><a href="" title="" class="write_link">本UI由魏宇轩开发,请勿盗用...</a></td>
					<td><a href="" title="" class="write_link">www.weiyuxuan.cn</a></td>
					<td><a href="" title="" class="write_link"><img src="./imgs/smImg.png" width="19" height="14"></a></td>
					<td><a href="" title="" class="write_link">公司新闻</a></td>
					<td>新建窗口</td>
					<td>
						<div class="switch">
							<div class="state" data-switch="1"></div>
							<span>正常</span>
						</div>
					</td>
					<td>2019-11-17 15:55:09</td>
					<td>
						<button class="button-small button-edit">编辑</button>
						<button class="button-small button-del">删除</button>
					</td>
				</tr>
			</table>
			<div class="actionBtnGroup">
				<button class="button-normal button-add mr10">全选</button>
				<button class="button-normal button-del">删除</button>
			</div>
		</div>
		<script>
			$(function() 
			{
				$('.button-del').click(function() 
				{
					popupAlert
					({
						title: "您真的要删除当前这个类目么？",
						info: "当前栏目AAAA123456",
						btnSure: "确定",
						btnCancel: "取消"
					}, function(sure) 
					{
						tipAlert
						({
							title: "删除成功",
							state: "true", //true成功 fail失败
							time: 1 //3秒后关闭
						});
					}, function(cancel) {});
				})
			})
		</script>
	</body>
</html>